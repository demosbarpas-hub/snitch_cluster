...
/* --- Διαμόρφωση Συμπίεσης & Macros --- */
#define JPEG_QUALITY 80      // Επίπεδο Συμπίεσης (1-100)
...
/* =========================================================================
   ΣΤΑΘΕΡΟΙ ΠΙΝΑΚΕΣ DCT & QUANTIZATION
========================================================================= */
...
void build_quant_tables(const uint8_t *base_table, float16_t *recip_fp16, double *recip_f64, int quality) {
    if (quality <= 0) quality = 1;
    if (quality > 100) quality = 100;
    int scale = (quality < 50) ? (5000 / quality) : (200 - 2 * quality);
    
    for (int i = 0; i < 64; i++) {
        int temp = ((int)base_table[i] * scale + 50) / 100;
        if (temp <= 0) temp = 1;
        if (temp > 255) temp = 255;
        
        recip_f64[i] = 1.0 / (double)temp;
...
void naive_chroma_420(double *in_cbcr, double *out_cb_f, double *out_cr_f, int w, int h) {
    for (int y = 0; y < h; y += 2) {
        for (int x = 0; x < w; x += 2) {
            ...
            out_cb_f[out_idx] = sum_cb; out_cr_f[out_idx] = sum_cr;
        }
    }
}
...
void opt_chroma_420_2d_ssr(double *in_cbcr, float16_t *out_cb_f, float16_t *out_cr_f) {
    ...
    void *p_top = (void*)(base_addr + 0); void *p_bot = (void*)(base_addr + 64);

    snrt_ssr_loop_2d(SNRT_SSR_DM0, 8, 8, 8, 128); snrt_ssr_read(SNRT_SSR_DM0, SNRT_SSR_2D, p_top);
    snrt_ssr_loop_2d(SNRT_SSR_DM1, 8, 8, 8, 128); snrt_ssr_read(SNRT_SSR_DM1, SNRT_SSR_2D, p_bot);
    snrt_ssr_enable();
    
    uint32_t alu_cb1, alu_cr1, alu_cb2, alu_cr2; uint32_t iters = 15;
    asm volatile(
        "fld      f31, 0(%[zero]) \n" "csrsi    0x801, 0x1 \n"      
        "1: \n"
        "fmv.d    f4, f0 \n"  "fmv.d    f5, f1 \n" "fmv.d    f8, f0 \n"  "fmv.d    f9, f1 \n"  
        ...
        "addi     %[iters], %[iters], -1 \n" "bgez     %[iters], 1b \n" "csrci    0x801, 0x1 \n"
        : [out_cb] "+r"(out_cb_f), [out_cr] "+r"(out_cr_f), [alu_cb1] "=&r"(alu_cb1), [alu_cr1] "=&r"(alu_cr1), [alu_cb2] "=&r"(alu_cb2), [alu_cr2] "=&r"(alu_cr2), [iters] "+r"(iters)
        : [zero] "r"(p_zero) : "f4", "f5", "f6", "f7", "f8", "f9", "f10", "f11", "f12", "f13", "f14", "f15", "f16", "f17", "f18", "f19", "f31", "memory"
    );
    snrt_ssr_disable();
}
...
void simd_dct_pass1(float16_t *in, float16_t *out, float16_t *tmp, const float16_t *dct_m_param) {
    ...
    uint64_t off_vec __attribute__((aligned(8))) = ((uint64_t)off16 << 48) | ((uint64_t)off16 << 32) | ((uint64_t)off16 << 16) | off16;
    snrt_ssr_loop_1d(SNRT_SSR_DM0, 16, 8); snrt_ssr_read(SNRT_SSR_DM0, SNRT_SSR_1D, (void*)in);
    snrt_ssr_loop_1d(SNRT_SSR_DM1, 16, 8); snrt_ssr_write(SNRT_SSR_DM1, SNRT_SSR_1D, (void*)out);
    snrt_ssr_enable();
    asm volatile("fld f10, 0(%0) \n" "frep.o %[n], 1, 0, 0 \n" "vfadd.h f1, f0, f10 \n" :: "r"(&off_vec), [n]"r"(15) : "f1", "f0", "f10", "memory");
    snrt_fpu_fence(); snrt_ssr_disable();

    snrt_ssr_loop_3d(SNRT_SSR_DM0, 2, 8, 8, 8, 0, 16); snrt_ssr_read(SNRT_SSR_DM0, SNRT_SSR_3D, (void*)out);
    snrt_ssr_loop_2d(SNRT_SSR_DM1, 16, 8, 8, 0);       snrt_ssr_read(SNRT_SSR_DM1, SNRT_SSR_2D, (void*)dct_m_param);
    snrt_ssr_enable();
    double zero_val = 0.0; double *p_zero = &zero_val;
...
void simd_dct_pass2(float16_t *in, float16_t *out, uint64_t *mat_bcast) {
    snrt_ssr_loop_2d(SNRT_SSR_DM0, 2, 64, 0, 8); snrt_ssr_read(SNRT_SSR_DM0, SNRT_SSR_2D, mat_bcast);
    snrt_ssr_loop_2d(SNRT_SSR_DM1, 16, 8, 8, 0); snrt_ssr_read(SNRT_SSR_DM1, SNRT_SSR_2D, in);
    snrt_ssr_loop_1d(SNRT_SSR_DM2, 16, 8);       snrt_ssr_write(SNRT_SSR_DM2, SNRT_SSR_1D, out);
    snrt_ssr_enable();
...
// =========================================================================
void naive_quant(double *in, double *out, const double *recip) {
    for (int i = 0; i < 64; i++) {
        out[i] = nearbyint(in[i] * recip[i]);
    }
}

void opt_quant_fp16(float16_t *in, float16_t *out, const float16_t *recip, uint64_t *magic_p) {
    snrt_ssr_loop_1d(SNRT_SSR_DM0, 16, 8); snrt_ssr_read(SNRT_SSR_DM0, SNRT_SSR_1D, (void*)in);
    snrt_ssr_loop_1d(SNRT_SSR_DM1, 16, 8); snrt_ssr_read(SNRT_SSR_DM1, SNRT_SSR_1D, (void*)recip);
    snrt_ssr_loop_1d(SNRT_SSR_DM2, 16, 8); snrt_ssr_write(SNRT_SSR_DM2, SNRT_SSR_1D, (void*)out);
    snrt_ssr_enable();
...
// =========================================================================
// MAIN ROUTINE (FULL PIPELINE: YCbCr -> Chroma -> DCT -> Quantization)
// =========================================================================
int main() {
    uint32_t core_id = snrt_cluster_core_idx();
    uintptr_t base = ALIGN_UP_TCDM((uintptr_t)snrt_l1_next());

    // --- Memory Allocation (S1 & S2) ---
    ...
    // --- Memory Allocation (S4 - Quantization) ---
    double    *naive_q_Y   = (double *)   base; base += TOTAL_PIXELS * sizeof(double);
    double    *naive_q_Cb  = (double *)   base; base += CHROMA_PIXELS * sizeof(double);
    double    *naive_q_Cr  = (double *)   base; base += CHROMA_PIXELS * sizeof(double);
    
    float16_t *opt_q_Y     = (float16_t *)base; base += TOTAL_PIXELS * sizeof(float16_t);
    float16_t *opt_q_Cb    = (float16_t *)base; base += CHROMA_PIXELS * sizeof(float16_t);
    float16_t *opt_q_Cr    = (float16_t *)base; base += CHROMA_PIXELS * sizeof(float16_t);

    float16_t *recip_luma_h   = (float16_t *)base; base += 64 * sizeof(float16_t);
    float16_t *recip_chroma_h = (float16_t *)base; base += 64 * sizeof(float16_t);
    double    *recip_luma_d   = (double *)   base; base += 64 * sizeof(double);
    double    *recip_chroma_d = (double *)   base; base += 64 * sizeof(double);

    uint64_t  *magic_vec_p = (uint64_t *) ALIGN_UP_TCDM(base);

    // Φόρτωση DCT Πινάκων μέσω DMA
    if (snrt_is_dm_core()) {
    ...
    if (snrt_is_compute_core() && core_id == 0) {
        ...
        // Setup 1: Προετοιμασία των Πινάκων Κβάντισης με βάση το JPEG_QUALITY
        build_quant_tables(std_luma_q, recip_luma_h, recip_luma_d, JPEG_QUALITY);
        build_quant_tables(std_chroma_q, recip_chroma_h, recip_chroma_d, JPEG_QUALITY);
        
        // Setup 2: Προετοιμασία Magic Vector
        float16_t magic_val = float_to_fp16(1536.0f);
        *magic_vec_p = ((uint64_t)magic_val << 48) | ((uint64_t)magic_val << 32) | 
                       ((uint64_t)magic_val << 16) | magic_val;

        // Setup 3: Mock Δεδομένα
        ...
        uint32_t n4_start = snrt_mcycle();
        for(int b = 0; b < 4; b++) naive_quant(&naive_dct_Y[b*64], &naive_q_Y[b*64], recip_luma_d);
        naive_quant(naive_dct_Cb, naive_q_Cb, recip_chroma_d);
        naive_quant(naive_dct_Cr, naive_q_Cr, recip_chroma_d);
        uint32_t n4_end = snrt_mcycle();
        ...
        uint32_t o4_start = snrt_mcycle();
        for(int b = 0; b < 4; b++) opt_quant_fp16(&opt_dct_Y[b*64], &opt_q_Y[b*64], recip_luma_h, magic_vec_p);
        opt_quant_fp16(opt_dct_Cb, opt_q_Cb, recip_chroma_h, magic_vec_p);
        opt_quant_fp16(opt_dct_Cr, opt_q_Cr, recip_chroma_h, magic_vec_p);
        uint32_t o4_end = snrt_mcycle();


        // ==========================================
        // VERIFICATION & DETAILED LOGGING
        // ==========================================
        int err_s2 = 0, err_dct = 0, err_q = 0;
        int max_prints = 10; 
        
        for (int i = 0; i < CHROMA_PIXELS; i++) {
            if (fp16_to_float(opt_cb_f[i]) - naive_cb_f[i] > 1.5 || fp16_to_float(opt_cb_f[i]) - naive_cb_f[i] < -1.5) err_s2++;
            if (fp16_to_float(opt_cr_f[i]) - naive_cr_f[i] > 1.5 || fp16_to_float(opt_cr_f[i]) - naive_cr_f[i] < -1.5) err_s2++;
        }
        
        for (int i = 0; i < TOTAL_PIXELS; i++) {
            if (fp16_to_float(opt_dct_Y[i]) - naive_dct_Y[i] > 3.0 || fp16_to_float(opt_dct_Y[i]) - naive_dct_Y[i] < -3.0) err_dct++;
        }
        for (int i = 0; i < CHROMA_PIXELS; i++) { 
            if (fp16_to_float(opt_dct_Cb[i]) - naive_dct_Cb[i] > 3.0 || fp16_to_float(opt_dct_Cb[i]) - naive_dct_Cb[i] < -3.0) err_dct++;
            if (fp16_to_float(opt_dct_Cr[i]) - naive_dct_Cr[i] > 3.0 || fp16_to_float(opt_dct_Cr[i]) - naive_dct_Cr[i] < -3.0) err_dct++;
        }

        printf("\n--- STAGE 4: QUANTIZATION ERRORS (Diff > 1.5) ---\n");
        // Check Quantization (Since inputs were different from S3, small divergences in integer rounding are expected)
        for (int i = 0; i < TOTAL_PIXELS; i++) {
            double diff = fp16_to_float(opt_q_Y[i]) - naive_q_Y[i];
            if (diff > 1.5 || diff < -1.5) {
                if (err_q < max_prints) printf(" Q_Y [%3d]: Naive = %5.0f | Opt = %5.0f | Diff = %5.1f\n", i, naive_q_Y[i], fp16_to_float(opt_q_Y[i]), diff);
                err_q++;
            }
        }
        for (int i = 0; i < CHROMA_PIXELS; i++) {
            double diff_cb = fp16_to_float(opt_q_Cb[i]) - naive_q_Cb[i];
            if (diff_cb > 1.5 || diff_cb < -1.5) {
                if (err_q < max_prints) printf(" Q_Cb[%3d]: Naive = %5.0f | Opt = %5.0f | Diff = %5.1f\n", i, naive_q_Cb[i], fp16_to_float(opt_q_Cb[i]), diff_cb);
                err_q++;
            }
            double diff_cr = fp16_to_float(opt_q_Cr[i]) - naive_q_Cr[i];
            if (diff_cr > 1.5 || diff_cr < -1.5) {
                if (err_q < max_prints) printf(" Q_Cr[%3d]: Naive = %5.0f | Opt = %5.0f | Diff = %5.1f\n", i, naive_q_Cr[i], fp16_to_float(opt_q_Cr[i]), diff_cr);
                err_q++;
            }
        }
        if (err_q == 0) printf(" None!\n");
        else if (err_q > max_prints) printf(" ... and %d more errors in Stage 4.\n", err_q - max_prints);
        ...
        printf("---------------------------------------------------\n");
        printf(" STAGE 2: CHROMA SUBSAMPLING (4:2:0)\n");
        printf("  - Naive Cycles : %u\n", n2_end - n2_start);
        printf("  - Opt   Cycles : %u\n", o2_end - o2_start);
        ...
        printf("---------------------------------------------------\n");
        printf(" STAGE 4: QUANTIZATION (4xY, 1xCb, 1xCr)\n");
        printf("  - Naive Cycles : %u\n", n4_end - n4_start);
        printf("  - Opt   Cycles : %u\n", o4_end - o4_start);
        printf("  - Speedup      : %.2fx\n", (float)(n4_end - n4_start) / (o4_end - o4_start));
        printf("---------------------------------------------------\n");
        uint32_t total_n = (n1_end - n1_start) + (n2_end - n2_start) + (n3_end - n3_start) + (n4_end - n4_start);
        uint32_t total_o = (o1_end - o1_start) + (o2_end - o2_start) + (o3_end - o3_start) + (o4_end - o4_start);
        printf(" TOTAL PIPELINE (S1 + S2 + S3 + S4)\n");
        ...
        printf(" VERIFICATION S4 (Quant)  : %s\n", (err_q == 0) ? "[SUCCESS]" : "[FAILED]");
        printf("===================================================\n\n");
    }
